...
const ZONE1: Record<number, number> = {
 ...
 * Returns null if weight is 0, negative, or > 70 lbs.
 */
export function getUspsZone1Rate(weightLbs: number): number | null {
...
/**
 * Savings on a single label vs USPS retail (Zone 1).
 * Returns 0 if weight is missing or retail rate is lower than price paid.
 */
export function labelSaving(weightLbs: number, pricePaid: number): number {
  const retail = getUspsZone1Rate(weightLbs);
  if (retail === null) return 0;
  return Math.max(0, retail - pricePaid);
}
